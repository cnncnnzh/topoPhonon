...
#     new_structure = model._make_supercell(1,2)
#     model_2d = model.cut_piece(1, 2)
#     q_path_2d = [[0.0,0.0],[0.5,0.5],[2/3,1/3],[0.0,0.0],[1/3,2/3]]
#     model_2d.atom_projected_band(q_path_2d, site_comb=[[0],[1]], node_names=node_names, k_num=80)
    
#     tp = Topology(model_2d)
#     wfs1 = tp.gen_circle_wfs([2/3,1/3],r=0.01)
#     wfs2 = tp.gen_circle_wfs([1/3,2/3],r=0.01)
#     print(tp.wilson_loop([0], wfs1,))
#     print(tp.wilson_loop([1], wfs1,))
#     print(tp.wilson_loop([0], wfs2,))
#     print(tp.wilson_loop([1], wfs2,))
#     print(tp.wilson_loop([5], wfs1,))
############# Graphene ################################################
    # lat=[[1,0], [1/2,np.sqrt(3)/2]]
    # coords = [[0,0],[1/3,1/3]]
    # masses = [1,1]
    # structure = Structure(2, lat=lat, coords=coords, masses=masses)
    # model = Model(structure)
    # c1 = 1
    # c2 = 0.1
    # model.set_fc(0, 0, [0,0], [0,0], [[3*c1+6*c2, 0],[0, 3*c1+6*c2]],)
    # model.set_fc(1, 1, [0,0], [0,0], [[3*c1+6*c2, 0],[0, 3*c1+6*c2]],)
    # #nearest neighbor
    # model.set_fc(0, 1, [0,0], [0,0], [[-c1, 0],[0, -c1]],)
    # model.set_fc(0, 1, [-1,0], [-1,0], [[-c1, 0],[0, -c1]],)
    # model.set_fc(0, 1, [0,-1], [0,-1], [[-c1, 0],[0, -c1]],)
    # #next nearest neighbor
    # model.set_fc(0, 0, [1,0], [1,0], [[-c2, 0],[0, -c2]],)
    # model.set_fc(0, 0, [-1,0], [-1,0], [[-c2, 0],[0, -c2]],)
    # model.set_fc(0, 0, [0,1], [0,1], [[-c2, 0],[0, -c2]],) 
    # model.set_fc(0, 0, [0,-1], [0,-1], [[-c2, 0],[0, -c2]],)
    # model.set_fc(0, 0, [1,-1], [1,-1], [[-c2, 0],[0, -c2]],)
    # model.set_fc(0, 0, [-1,1], [-1,1], [[-c2, 0],[0, -c2]],)
    # #nearest neighbor
    # model.set_fc(1, 0, [0,0], [0,0], [[-c1, 0],[0, -c1]],)
    # model.set_fc(1, 0, [-1,0], [-1,0], [[-c1, 0],[0, -c1]],)
    # model.set_fc(1, 0, [0,-1], [0,-1], [[-c1, 0],[0, -c1]],)
    # #next nearest neighbor
    # model.set_fc(1, 1, [1,0], [1,0], [[-c2, 0],[0, -c2]],)
    # model.set_fc(1, 1, [-1,0], [-1,0], [[-c2, 0],[0, -c2]],)
    # model.set_fc(1, 1, [0,1], [0,1], [[-c2, 0],[0, -c2]],) 
    # model.set_fc(1, 1, [0,-1], [0,-1], [[-c2, 0],[0, -c2]],)
    # model.set_fc(1, 1, [1,-1], [1,-1], [[-c2, 0],[0, -c2]],)
    # model.set_fc(1, 1, [-1,1], [-1,1], [[-c2, 0],[0, -c2]],)
    
    # node_names=[r"$\Gamma$", "M", "K", r"$\Gamma$", "K'"]
    # q_path = [[0.0,0.0],[0.5,0.5],[2/3,1/3],[0.0,0.0],[1/3,2/3]]
    # model.atom_projected_band(q_path,site_comb=[[1],[2]])
    
    # tp = Topology(model)
    # wfs = tp.gen_circle_wfs([0.334,0.334],r=0.03)
    # bp_1 = tp.wilson_loop([0,1], wfs)
    # # bp_2 = tp.wcc([0,1], wfs)
    # print(bp_1)     
    
############## 2 atoms, layered #########################
    # lat=[[2,0],[0,1]]
    # coords = [[0.0,0.0],[0.5,0.0]]
    # masses = [1,1]
    # structure = Structure(2, lat=lat, coords=coords, masses=masses)
    
    # model = Model(structure)
    # c0, c1, c2, c3, c4 =  1, 3, 2, 2, 0.8
    # model.set_fc( 0, 0, [0,0], [0,0], [[c0+c3+c4, 0],[0, c0+c1+c4]],)
    # model.set_fc( 0, 0, [0,1], [0,1], [[0,0],[0,-c1]],)
    # model.set_fc( 0, 0, [0,-1], [0,-1], [[0,0],[0,-c1]],)
    # model.set_fc( 1, 1, [0,0], [0,0], [[c0+c3+c4, 0],[0, c0+c2+c4]],)
    # model.set_fc( 1, 1, [0,1], [0,1], [[0,0],[0,-c2]],)
    # model.set_fc( 1, 1, [0,-1], [0,-1], [[0,0],[0,-c2]],)
    # model.set_fc( 0, 1, [0.5,0], [0,0], [[-c3,0],[0,0]],)
    # model.set_fc( 0, 1, [-0.5,0], [-1,0], [[-c3,0],[0,0]],)
    # model.set_fc( 0, 1, [0.5,0.5], [0,0], [[-c4,0],[0,-c4]],)  
    # model.set_fc( 0, 1, [-0.5,0.5], [-1,0], [[-c4,0],[0,-c4]],)
    # model.set_fc( 0, 1, [0.5,-0.5], [0,-1], [[-c4,0],[0,-c4]],)
    # model.set_fc( 0, 1, [-0.5,-0.5], [-1,-1], [[-c4,0],[0,-c4]],)
    # model.set_fc( 1, 0, [0.5,0], [1,0], [[-c3,0],[0,0]],)
    # model.set_fc( 1, 0, [-0.5,0], [0,0], [[-c3,0],[0,0]],)
    # model.set_fc( 1, 0, [0.5,0.5], [1,0], [[-c4,0],[0,-c4]],)  
    # model.set_fc( 1, 0, [-0.5,0.5], [0,0], [[-c4,0],[0,-c4]],)
    # model.set_fc( 1, 0, [0.5,-0.5], [1,-1], [[-c4,0],[0,-c4]],)
    # model.set_fc( 1, 0, [-0.5,-0.5], [0,-1], [[-c4,0],[0,-c4]],)
    
    # q_path = [[0.5,0.5], [0.0,0.0], [0.5,0.0],[0.0,0.5],[0.0,0.0]]
    # model.atom_projected_band(q_path,
    #                         site_comb=[[1],[2]],)
    # q_points, q_dist, node_dist = model.make_q_path(q_path, 101)    
