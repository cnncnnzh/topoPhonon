...
model = read_from_files(r"..\tests\LiCaAs")
# specify the k path
...
######### BeAu ##############
print("BeAu!!")
model = read_from_files(r"..\tests\BeAu")
...
model_2d.atom_projected_band(
    q_path_2d,
    node_names=["X", "Y", "X", "$\Gamma$", "M", "X", "Y", "X"],
    k_num=70,
    y_min=100,
    y_max=126,
    margin_highlight=[0.45, 0],
    unit="cm-1",
)
# plot the gaussian smeared surface band structure
model_2d.plot_edge(
    q_path_2d, [1, 0], y_min=100, y_max=126, k_num=150, fin_dirc=None, unit="cm-1"
)


######### graphene ##############
print("Graphene!!")
model = read_from_files(r"..\tests\graphene")
# make the model 2d!
model_2d = model.cut_piece(1, 2)
node_names = [r"$\Gamma$", "M", "K", r"$\Gamma$", "K'"]
k_path_2d = [[0.0, 0.0], [0.5, 0.5], [2 / 3, 1 / 3], [0.0, 0.0], [1 / 3, 2 / 3]]
model_2d.atom_projected_band(
    k_path_2d, site_comb=[[0], [1]], node_names=node_names, k_num=80
)
...
model_1d.plot_edge(k_path_1d, [0, 1], y_min=6, y_max=30, k_num=100, fin_dirc=1, sigma=3)
model_1d.atom_projected_band(
    k_path_1d, y_min=0, y_max=50, fin_dirc=1, margin_highlight=[0, 1]
)
...
unitcell = r"..\tests\SrTiO3\POSCAR"
supercell = r"..\tests\SrTiO3\SPOSCAR"
born = r"..\tests\SrTiO3\BORN"
force_const = r"..\tests\SrTiO3\FORCE_CONSTANTS"
ph = phonopy.load(
    supercell_filename=supercell,
    born_filename=born,
    force_constants_filename=force_const,
)
# create DynamicalMatrix object
dm = get_dynamical_matrix(
    ph.force_constants, ph.supercell, ph.primitive, nac_params=ph.nac_params
)
